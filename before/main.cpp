...
int main()
{
	...
	while (1)
	{
		...
		vector<Vec3f> circles; //存储HoughCircles输出的圆 圆以圆心半径:Vec3f(x,y,r)表示

		HoughLinesP(binImage, lines, 1, CV_PI / 180, 50, 50, 10);				   //直线检测
		HoughCircles(grayImage, circles, HOUGH_GRADIENT, 1.5, 50, 200, 150, 0, 0); //圆检测(不能识别同心圆)
		...
		//寻找中线
		PolarLineTypeDef tmp = MiddleLine(lines);
